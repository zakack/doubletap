/*
 * osu-interceptd - Standalone SOCD-cleaning daemon for osu! and similar
 *                  rhythm games.
 *
 ...
 *
 * It replaces the legacy `intercept | osu-intercept | uinput` pipeline:
 * no `interception-tools` dependency at runtime.
 *
 * License: same as the rest of this project (see LICENSE.md).
 */
...
static void
logf_(const char *level, const char *fmt, ...)
...
/* ------------------------------------------------------------------------- */
/* Audio (PipeWire) - preserved from the original osu-intercept.c            */
/* ------------------------------------------------------------------------- */
...
static yaml_node_t *
ynode(yaml_document_t *doc, int id)
{
...
static int
scalar_dup(yaml_node_t *n, char **out)
{
...
static int
parse_bool(yaml_node_t *n, int *out)
{
	if (!n || n->type != YAML_SCALAR_NODE) return -1;
...
static int
parse_key_code(yaml_node_t *n, int *out)
{
	if (!n || n->type != YAML_SCALAR_NODE) return -1;
	const char *s = (const char *)n->data.scalar.value;
...
static int
load_config(const char *path, oid_config_t *c)
{
...
typedef struct {
	...
	int               fd;
	char             *path;
	/* Per-device SOCD state (matches the original osu-intercept.c) */
	int               k1;   /* physical k1 currently down? */
	int               k2;   /* physical k2 currently down? */
	int               act;  /* 1 == v1 currently active, 0 == v2 (or none) */
	int               last_was_k1; /* was the SINGLE press k1 (1) or k2 (0)? */
} input_dev_t;

static int
input_open(const char *path, input_dev_t *out)
{
...
/* Build a virtual keyboard whose KEY capability set is the union of every
 * source device's KEY codes, plus the configured v1/v2. EV_SYN is always
 * enabled. We do NOT clone EV_REL/EV_ABS/EV_LED: pure keyboards. */
static int
build_virtual(input_dev_t *inputs, size_t n, const oid_config_t *cfg)
{
	vdev = libevdev_new();
	if (!vdev) {
 ...
 * audio), 0 otherwise.
 *
 * Logic (transcribed from the original osu-intercept.c):
 *
 *   state = k1 + k2 + ie.value   (k1, k2 reflect post-update physical state)
 *
...
static int
process_event(input_dev_t *in, const struct input_event *ie,
              const oid_config_t *cfg)
{
	/* Drop scancodes; the original pipeline stripped these too. */
	if (ie->type == EV_MSC && ie->code == MSC_SCAN)
		return 0;

	/* Pass-through everything that isn't k1/k2. */
	if (ie->type != EV_KEY ||
	...
	/* Ignore key auto-repeat (value == 2). */
	if (ie->value == 2)
		return 0;
	...
	default:
		/* Shouldn't happen with sane inputs. */
		break;
	}
...
/* Drain everything libevdev has buffered for `in`, looping over SYNC drops
 * as needed. Returns 0 on normal EAGAIN, -1 on fatal error (device gone). */
static int
drain_device(input_dev_t *in, const oid_config_t *cfg)
{
...
static int
run_loop(input_dev_t *inputs, size_t n_inputs, const oid_config_t *cfg)
{
...
main(int argc, char **argv)
{
	...
	/* Best-effort audio init (legacy behavior: warn and continue). */
	...
	/* Build a single virtual keyboard mirroring all sources' caps. */
